...
const cleanRepsText = (reps) => reps ? reps.replace(/^\d+\sSet\sx\s*/i, '') : '';
...
className={⁠px-3 py-1.5 rounded-full text-xs font-semibold transition-all flex items-center gap-1.5 ${ currentView === 'templates'  ? 'bg-amber-500 text-black font-bold'  : 'bg-zinc-800/90 text-zinc-200 hover:bg-zinc-700' }⁠}
>
...
className={⁠w-7 h-7 rounded-full flex items-center justify-center border text-xs check-anim active:scale-90 ${ day.isRest  ? 'border-zinc-700 bg-zinc-900 text-zinc-500'  : dayCompleted  ? 'border-amber-500 bg-amber-500 text-black font-bold'  : 'border-amber-500/60 hover:border-amber-500 text-transparent' }⁠}
>
{day.isRest ? '😴' : dayCompleted ? <i className="fa-solid fa-check"></i> : <i className="fa-solid fa-check opacity-0 hover:opacity-50"></i>}
</button>
<span className={⁠font-bold tracking-wide text-base ${ day.isRest ? 'text-zinc-500 italic' : dayCompleted ? 'text-zinc-400 line-through decoration-amber-500/50' : 'text-zinc-100' }⁠}>
...
className={⁠px-2.5 py-1 rounded-lg text-xs font-semibold border flex items-center gap-1 transition-all ${ isCollapsed  ? 'bg-amber-500/20 text-amber-400 border-amber-500/40'  : 'bg-zinc-900 text-zinc-400 border-zinc-800 hover:text-zinc-200' }⁠}
...
className={⁠px-2 py-2.5 rounded-xl border text-xs font-bold flex items-center justify-between transition-all active:scale-95 cursor-pointer touch-manipulation select-none min-w-0 ${ isSlotChecked ? 'bg-amber-500/20 border-amber-500 text-amber-300 shadow-[0_0_12px_rgba(234,179,8,0.25)]' : 'bg-zinc-900/90 border-zinc-800 text-zinc-400 hover:border-zinc-700' }⁠}
>
...
<span className={⁠w-4 h-4 sm:w-5 sm:h-5 rounded-full flex items-center justify-center border text-[9px] sm:text-[11px] font-extrabold shrink-0 ml-0.5 transition-all ${ isSlotChecked  ? 'bg-amber-500 border-amber-500 text-black shadow-sm'  : 'border-zinc-600 text-transparent' }⁠}>
✓
</span>
</button>
);
...
>
<div className="flex items-center space-x-3">
<span className={⁠w-5 h-5 rounded-full flex items-center justify-center border transition-all text-xs ${ isChecked  ? 'border-green-500 bg-green-500/20 text-green-400 font-bold'  : 'border-zinc-600 text-zinc-700 group-hover/item:border-zinc-400' }⁠}>
{isChecked ? '✓' : '○'}
</span>
<span className={⁠text-sm font-medium transition-all ${ isChecked  ? 'text-zinc-500 line-through decoration-zinc-600'  : 'text-zinc-200' }⁠}>
...
className={⁠px-2.5 py-1.5 rounded-lg text-xs font-semibold border flex items-center gap-1 transition-all ${ isTplCollapsed  ? 'bg-amber-500/20 text-amber-400 border-amber-500/40'  : 'bg-zinc-800 text-zinc-400 border-zinc-700 hover:text-zinc-200' }⁠}
...
className={⁠h-9 flex items-center justify-center rounded-lg hover:bg-zinc-800 text-lg transition-colors ${ activeDayModal.day.emoji === emoji ? 'bg-amber-500/20 border border-amber-500' : '' }⁠}
>
{emoji}
</button>
))}
...
className={⁠w-6 h-6 rounded-md border flex items-center justify-center transition-all ${ activeDayModal.day.isRest  ? 'bg-amber-500 border-amber-500 text-black font-extrabold'  : 'bg-zinc-900 border-zinc-600' }⁠}
>
...
className={⁠flex items-center justify-between p-3.5 rounded-2xl border cursor-pointer transition-all ${ isSelected  ? 'bg-amber-500/10 border-amber-500/80 text-white shadow-[0_0_15px_rgba(234,179,8,0.1)]'  : 'bg-zinc-950 border-zinc-800 hover:border-zinc-700 text-zinc-300' }⁠}
>
<div className="flex items-center space-x-3">
...
<span className={⁠px-3 py-1.5 rounded-xl text-xs font-extrabold flex items-center gap-1 transition-all ${ isSelected  ? 'bg-amber-500 text-black shadow-sm'  : 'bg-zinc-900 border border-zinc-800 text-zinc-400' }⁠}>
...
className={⁠h-10 flex items-center justify-center rounded-xl hover:bg-zinc-800 text-xl transition-all active:scale-90 ${ activeTemplateModal.emoji === emoji ? 'bg-amber-500/20 border border-amber-500 text-2xl shadow-sm' : '' }⁠}
>
{emoji}
</button>
))}
...
className={⁠py-2 px-1 rounded-xl font-extrabold text-xs border transition-all ${ isSelected ? 'bg-amber-500 border-amber-500 text-black shadow-md' : 'bg-zinc-950 border-zinc-800 text-zinc-300 hover:border-zinc-700' }⁠}
>
{setNum} SET
</button>
);
...
className={⁠px-3 py-1.5 rounded-xl border text-xs font-bold transition-all ${ isPresetActive  ? 'bg-amber-500/20 border-amber-500 text-amber-300'  : 'bg-zinc-950 border-zinc-800 text-zinc-300 hover:border-zinc-700' }⁠}
>
{starPreset}
</button>
);
...
// Fungsi render automatik yang disesuaikan untuk memastikan tiada masalah skrin gelap di GitHub Pages
function renderApp() {
